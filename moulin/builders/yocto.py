...
def gen_build_rules(generator: ninja_syntax.Writer):
    ...
    # Update bblayers.conf with own variables
    cmd = " && ".join([
        "cd $yocto_dir",
        "echo '# Code generated by moulin. All manual changes will be lost' > $work_dir/conf/bblayers_moulin.conf",
        "for x in $conf; do echo $$x >> $work_dir/conf/bblayers_moulin.conf; done",
        "sed \"/require bblayers_moulin\\.conf/d\" -i $work_dir/conf/bblayers.conf",
        "echo 'require bblayers_moulin.conf' >> $work_dir/conf/bblayers.conf",
    ])
    generator.rule(
        "yocto_update_bblayersconf",
        command=cmd,
        description="Update bblayers.conf",
    )
    generator.newline()
...
class YoctoBuilder:
    ...
    def gen_build(self):
        ...
        # Update local variables in bblayers.conf
        layers_conf_target = os.path.join(self.yocto_dir, self.work_dir, "conf",
                                          "bblayers_moulin.conf")
        layers_conf_node = self.conf.get("layers_conf", None)
        if layers_conf_node:
            layers_conf = _flatten_yocto_conf(layers_conf_node)
        else:
            layers_conf = []
        # '$' is a ninja escape character so we need to quote it
        layers_conf_lines = [
            shlex.quote(f'{k.replace("$", "$$")} = "{v.replace("$", "$$")}"')
            for k, v in layers_conf
        ]

        self.generator.build(layers_conf_target,
                             "yocto_update_bblayersconf",
                             env_target,
                             variables=dict(common_variables, conf=" ".join(layers_conf_lines)))
